...
# In case your text data is separated in sentences and has a sentence ID variable
compiled_data <- data %>%
  group_by(Text_ID) %>%
  summarize(Compiled_Interview_Text = paste(Text, collapse = " "))
...
tweets_for_topic <- tweets$your_tweet_text_variable


# Calculate the sentiment scores for each tweet
sentiment_scores <- sapply(preprocessed_tweets, function(tweet) {
  words <- unlist(str_split(tweet, "\\s+"))
  total_score <- sum(afinn$score[match(words, afinn$word)], na.rm = TRUE)
  sentiment <- polarity(mean(total_score), length(tweet))
  sentiment$all$total
})

# Calculate the overall sentiment score for the topic based on the ratio of sum(scores) to total character length.
overall_sentiment_score <- sum(sentiment_scores) / sum(nchar(tweets_for_topic))

# Normalize the sentiment score to [0, 1] range
min_score <- 0  # Replace with the minimum possible sentiment score among all topics.
max_score <- 1  # Replace with the maximum possible sentiment score among all topics.
normalized_sentiment_score <- (overall_sentiment_score - min_score) / (max_score - min_score)

# Print the normalized sentiment score
cat(normalized_sentiment_score, "\n")
